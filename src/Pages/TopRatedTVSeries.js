...
const TopRatedSeries = () => {
  ...
  const fetchSeries = async (page) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/tv/top_rated`, {
        params: {
      ...
      console.error('Error fetching the popular series:', error);
      setLoading(false);
      setLoadingMore(false);
    }
  };
  ...
  return (
    <div className="series-container mt-4">
      {loading ? (
        ...
        <>
          <h1 className="series-title">Top Rated Series</h1>
