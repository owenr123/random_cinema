...
const ComingSoonMovies = () => {
  ...
  const fetchMovies = async (page) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/movie/upcoming`, {
        params: {
      ...
      console.error('Error fetching the coming soon movies:', error);
      setLoading(false);
      setLoadingMore(false);
    }
  };
  ...
  return (
    <div className="movies-container mt-4">
      {loading ? (
        ...
        <>
          <h1 className="movies-title">Coming Soon Movies</h1>
